...
- ✅ **Infrastructure Integration Phase 1 Complete** - Database, MinIO, Redis startup integration added
- ✅ SQLAlchemy metadata field conflict resolved (renamed to `doc_metadata`)
- ✅ Pydantic settings updated with all infrastructure configuration fields
- ✅ Health endpoint enhanced to check database, MinIO, and Redis connectivity
- ✅ All infrastructure services tested and verified healthy
...
python -c "import asyncio; from backend.database.connection import check_db_health; from backend.storage.minio_client import check_minio_health; from backend.cache.redis_client import check_redis_health; async def test(): print('Database:', '✅' if await check_db_health() else '❌'); print('MinIO:', '✅' if await check_minio_health() else '❌'); print('Redis:', '✅' if await check_redis_health() else '❌'); asyncio.run(test())"
...
- **SQLAlchemy metadata conflict**: If you see "Attribute name 'metadata' is reserved", the database models use `doc_metadata` field instead of `metadata` to avoid conflicts.
- **Missing database dependencies**: Install required dependencies with `pip install -r requirements.txt` (includes asyncpg, sqlalchemy, redis, minio packages).
- **Pydantic validation errors for infrastructure settings**: Ensure all infrastructure config fields are defined in `backend/settings.py`. Missing fields: DATABASE_URL, REDIS_URL, MINIO_* settings, JWT_* settings.
- **Database connection failures**: Check that PostgreSQL is running with correct credentials in .env. Test with: `python -c "import asyncio; from backend.database.connection import check_db_health; print(asyncio.run(check_db_health()))"`
- **MinIO connection failures**: Verify MinIO is running on correct port. Test with: `python -c "import asyncio; from backend.storage.minio_client import check_minio_health; print(asyncio.run(check_minio_health()))"`
- **Redis connection failures**: Ensure Redis is running with correct password. Test with: `python -c "import asyncio; from backend.cache.redis_client import check_redis_health; print(asyncio.run(check_redis_health()))"`
- **Port 8000 already in use**: Kill existing backend processes: `pkill -f "python.*main.py"` or `lsof -i :8000` and kill specific PIDs.
- **FastAPI deprecation warnings**: `on_event` is deprecated in favor of lifespan handlers. This doesn't affect functionality but should be updated for production.
- **Infrastructure services not starting**: Use `docker compose -f docker-compose.dev.yml up -d` to start PostgreSQL, MinIO, and Redis services.
...
- **Changelog tracking:** Every code/file change MUST be logged in docs/project/changelog.md with date and time (format: YYYY-MM-DD HH:MM:SS)
- **Task tracking:** Mark tasks as complete in docs/project/tasks.md when finished
